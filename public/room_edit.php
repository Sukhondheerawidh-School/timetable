...
<div class="max-w-xl mx-auto px-4">
  ...
  <form method="post" class="bg-white rounded-2xl shadow border border-slate-200 p-6 space-y-4">
    ...
    <div>
      ...
      <input name="room_code" class="w-full border border-slate-200 rounded-xl px-3 py-2 bg-white focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 outline-none transition text-sm" required value="<?= htmlspecialchars($_POST['room_code'] ?? $room['room_code']); ?>">
    </div>
    <div>
      <label class="block text-sm font-medium text-slate-700 mb-1.5">ชื่อห้อง</label>
      <input name="room_name" class="w-full border border-slate-200 rounded-xl px-3 py-2 bg-white focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 outline-none transition text-sm" required value="<?= htmlspecialchars($_POST['room_name'] ?? $room['room_name']); ?>">
    </div>
    <div>
      <label class="block text-sm font-medium text-slate-700 mb-1.5">อาคาร</label>
      <input name="building" class="w-full border border-slate-200 rounded-xl px-3 py-2 bg-white focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 outline-none transition text-sm" value="<?= htmlspecialchars($_POST['building'] ?? $room['building']); ?>">
    </div>
    <div>

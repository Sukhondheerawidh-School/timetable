<?php
/**
 * shift_export.php — รายงานเวรวันหยุดปกติ (พิมพ์ / PDF)
 */
...
/* ── helpers ── */
function she_h($s): string {
    return htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8');
}
function she_date_th(?string $ymd): string {
    if (!$ymd) return '';
...
/* ── Query report data ── */
$sql = 'SELECT d.id AS date_id, d.duty_date, d.note,
...
$params = [$year_id];
if ($building_id > 0) {
    $sql .= ' AND (a.building_id=? OR a.building_id IS NULL)';
    $params[] = $building_id;
}
$sql .= ' ORDER BY d.duty_date, b.sort_order, b.building_name, t.teacher_code, t.first_name';

$rStmt = $pdo->prepare($sql);
$rStmt->execute($params);
$rRows = $rStmt->fetchAll(PDO::FETCH_ASSOC);

/* Group: date → {note, groups: {building_name => [items]}} */
$report = [];
foreach ($rRows as $rr) {
    $dt = $rr['duty_date'];
    if (!isset($report[$dt])) {
        $report[$dt] = ['note' => $rr['note'], 'groups' => []];
    }
...
$backUrl     = url('shift.php') . '?tab=report&year_id=' . $year_id;
?>
<!DOCTYPE html>
<html lang="th">
<head>
...
<title>รายงานเวรวันหยุดปกติ – <?= she_h($yearLabel) ?></title>
<style>
  :root {
    --head-bg: #f0fdf4;
    --border:  #d1d5db;
  ...
  * { box-sizing: border-box; margin:0; padding:0; }
  ...
  .bld-link.active { background:#059669; color:#fff; border-color:#059669; }
  ...
  th.col-note { width:150px; text-align:left; }
  .bld-tag { display:inline-block; font-size:11px; font-weight:700; color:#059669; margin-right:6px; }
  ...
  table.summary td.cnt { text-align:center; font-weight:700; color:#059669; }
...
<div class="paper">
  ...
  <div class="top">
    <div>
      <div class="title">🗓️ รายงานเวรวันหยุดปกติ</div>
      <div class="sub">
        ปีการศึกษา: <?= she_h($yearLabel ?: (string)$year_id) ?>
        <?= $buildingLabel ? ' &nbsp;·&nbsp; อาคาร: ' . she_h($buildingLabel) : '' ?>
        &nbsp;·&nbsp; วันที่พิมพ์: <?= $today ?>
      </div>
    </div>
    <div class="actions">
      <button class="btn primary" onclick="window.print()">🖨 พิมพ์ / PDF</button>
      <a class="btn" href="<?= she_h($backUrl) ?>">← กลับ</a>
  ...
  <!-- Summary cards -->
  <div class="cards">
    <div class="card"><div class="card-num indigo"><?= $totalDates ?></div><div class="card-lbl">วันทั้งหมด</div></div>
  ...
  <div class="section">
    ...
    <table>
      <thead>
        <tr>
          <th class="col-date" style="text-align:left;">วันที่</th>
          <th class="col-note" style="text-align:left;">หมายเหตุ</th>
      ...
      <tbody>
        ...
        <tr>
          <td>
            <div style="font-weight:600;"><?= she_h(she_date_th($dt)) ?></div>
          </td>
          <td style="color:var(--muted);font-size:12px;"><?= she_h($dr['note'] ?? '') ?></td>
          <td>
            ...
            ?>
              <?php if ($showGrp): ?><span class="bld-tag">🏫 <?= she_h($grpName) ?></span><?php endif; ?>
              <?php foreach ($items as $it):
                $hasSub = !empty($it['to_teacher_id']);
              ?>
                <span class="chip <?= $hasSub ? 'chip-sub' : 'chip-ok' ?>">
                  <?= she_h($it['first_name'] . ' ' . $it['last_name']) ?>
                  <?php if ($hasSub): ?>
                    <span class="arrow">→ <?= she_h($it['sub_first'] . ' ' . $it['sub_last']) ?></span>
                  <?php endif; ?>
                </span>
              <?php endforeach; ?>
  ...
  <div class="section">
    ...
    <table class="summary">
      <thead>
        <tr>
          <th>ชื่อครู</th>
          <?php if ($hasBuildings): ?><th style="width:160px;">อาคาร</th><?php endif; ?>
          <th style="width:80px;">จำนวนเวร</th>
        </tr>
      </thead>
      <tbody>
        <?php foreach ($tSummary as $ts): ?>
        <tr>
          <td><?= she_h($ts['name']) ?></td>
          <?php if ($hasBuildings): ?><td style="font-size:12px;color:var(--muted);"><?= she_h($ts['building']) ?></td><?php endif; ?>
  ...
  <div style="margin-top:24px;text-align:right;font-size:11px;color:var(--muted);">
    พิมพ์เมื่อ <?= $today ?> · ระบบบริหารตารางสอน
  </div>

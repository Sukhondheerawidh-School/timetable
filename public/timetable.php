    ...
    if ($action==='add') {
      try {
        ...
        if (!empty($L['homeroom_room_id'])) {
          ...
          $stBldg->execute([(int)$L['homeroom_room_id']]);
...
<!-- Modal ยืนยันการลงวิชาซ้ำ / ข้ามตึก -->
<?php if ($confirmData): ?>
<div id="confirmModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
  <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
    <div class="flex items-start gap-3 mb-4">
      ...
      <div class="flex-1">
        <h3 class="text-lg font-semibold text-slate-900 mb-2">⚠️ พบข้อควรระวัง</h3>
        <p class="text-sm text-slate-500 mb-3">วิชา <strong class="text-slate-800"><?= htmlspecialchars($confirmData['subject']); ?></strong> / วัน<strong><?= htmlspecialchars($confirmData['day_name']); ?></strong></p>
        <ul class="space-y-1.5 mb-1">
          <?php foreach (($confirmData['warnings'] ?? []) as $w): ?>
          <li class="text-sm <?= $w['type']==='cross_building' ? 'text-rose-700' : 'text-amber-700'; ?> bg-<?= $w['type']==='cross_building' ? 'rose' : 'amber'; ?>-50 rounded-lg px-3 py-2">
            <?= htmlspecialchars($w['msg']); ?>
          </li>
          <?php endforeach; ?>
        </ul>
        <p class="text-sm text-slate-600 mt-3">คุณต้องการลงคาบนี้ต่อหรือไม่?</p>
      </div>
    </div>
...
/* ✅ สีการ์ด AUTO (ฟ้า) vs MANUAL (เขียว) */
.slot-card.slot-auto {
  border-color: #93c5fd;
  background: #eff6ff;
}
.slot-card.slot-auto:hover {
  background: #dbeafe;
}
.slot-card.slot-manual {
  border-color: #6ee7b7;
  background: #f0fdf4;
}
.slot-card.slot-manual:hover {
  background: #dcfce7;
}

/* ✅ Details animation */
details[open] > form.add-form {
...
<div class="max-w-7xl mx-auto px-4 mt-8">
  ...
  <?php if($err): ?><div class="mb-3 p-3 rounded-xl flex items-start gap-2 bg-rose-50 text-rose-700 border border-rose-200"><span class="text-lg">&#x274c;</span><span><?= htmlspecialchars($err); ?></span></div><?php endif; ?>

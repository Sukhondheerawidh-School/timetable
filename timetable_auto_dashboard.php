...
require __DIR__ . '/public/timetable_auto_dashboard.php';
exit;
...
<div class="max-w-6xl mx-auto px-4 mt-8">
  ...
  <div class="bg-white rounded-2xl shadow overflow-hidden">
    <div class="px-4 py-3 font-medium border-b">สถานะการลงคาบ (รวมตาม ห้อง × วิชา)</div>
    <div class="overflow-x-auto">
      <table class="min-w-full text-sm">
        <thead class="bg-slate-50">
          <tr>
            <th class="text-left px-3 py-2">ห้อง</th>
            <th class="text-left px-3 py-2">วิชา</th>
            <th class="text-right px-3 py-2">ใช้ไป</th>
        ...
        <tbody>
          <?php foreach($summary as $row): ?>
            <tr class="border-t">
              <td class="px-3 py-2"><?= htmlspecialchars($row['class_name']) ?></td>
              <td class="px-3 py-2"><?= htmlspecialchars($row['label']) ?></td>
              <td class="px-3 py-2 text-right"><?= (int)$row['used_cnt'] ?></td>
              <td class="px-3 py-2 text-right"><?= (int)$row['quota'] ?></td>
              <td class="px-3 py-2 text-right <?= (int)$row['remain']>0?'text-emerald-600':'text-slate-500' ?>">
                <?= (int)$row['remain'] ?>
              </td>
            </tr>
          <?php endforeach; ?>

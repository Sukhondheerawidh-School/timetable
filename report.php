<?php
// Compatibility entrypoint: serve the actual page from /public
...
// ✅ กำหนดค่าเริ่มต้นเทอมตามเดือนปัจจุบันถ้าไม่ส่งมา
if (isset($_GET['term_no']) && $_GET['term_no'] !== '') {
  $term_no = (int)$_GET['term_no'];
  if ($term_no !== 1 && $term_no !== 2) $term_no = 1;
} else {
  $currentMonth = (int)date('n');
  // เดือน 4-9 (เมษายน-กันยายน) = เทอม 1
  // เดือน 10-3 (ตุลาคม-มีนาคม) = เทอม 2
  $term_no = ($currentMonth >= 4 && $currentMonth <= 9) ? 1 : 2;
}
...
$term_text = $_GET['term_text'] ?? ('ภาคเรียนที่ '.$term_no);
...
@media print{
  ...
  *{-webkit-print-color-adjust:exact;print-color-adjust:exact}
  ...
  .sign-box{border:1px solid #000!important;border-radius:0!important}
...
/* --------------------------
   สร้าง HTML
---------------------------*/
function buildReportHTML($pages,$periods,$base_days,$subjectCodeByName,$screenCss,$school_name,$printed_at,$logoVers,$hasLogo,$show_subject,$show_code,$show_room,$show_teacher,$for_pdf=false,$year_text='',$term_text='',$show_room_code=false,$roomCodeById=[], $director_name='', $director_date='', $directorSignVers='', $hasDirectorSign=false, $show_approval=true){
  ob_start();
  if (!$for_pdf): ?>
            ...
            <div class="sign-row">
              <div class="sign-box approve-box">
                ...
                <div class="sign-meta">
                  ...
                  <span>วันที่ <?= h($director_date !== '' ? thaiDateTextFromDMY($director_date) : '....../....../......') ?></span>
                </div>
              </div>
            </div>
          </div>
...
if ($export==='pdf'){
  ...
  if (count($pages) === 0) {
    ...
    echo '<li><strong>4.</strong> จึงค่อยกดปุ่ม <strong class="bg-yellow-200 px-2 py-1 rounded">"Export PDF"</strong></li>';
    echo '</ol>';
    ...
    <form method="get" id="filterForm" class="p-5 grid md:grid-cols-6 grid-cols-2 gap-4 items-end bg-slate-50 rounded-b-2xl">
      ...
      <div>
        ...
        <select name="term_no" id="f_term" class="w-full mt-1 rounded-xl border-gray-300 bg-white">
          <option value="1" <?= $term_no===1?'selected':''; ?>>เทอม 1</option>
          <option value="2" <?= $term_no===2?'selected':''; ?>>เทอม 2</option>
        </select>
      </div>
      ...
      <div class="md:col-span-6 flex flex-wrap items-center gap-5">
        ...
        <button class="rounded-xl bg-indigo-600 text-white px-4 py-2" type="submit">แสดงตัวอย่าง</button>
        <a class="rounded-xl bg-slate-700 text-white px-4 py-2"
           href="<?= h($_SERVER['PHP_SELF'].'?'.http_build_query(array_merge($_GET,['__apply'=>1,'export'=>'print']))) ?>">พิมพ์</a>
        <a class="rounded-xl bg-emerald-600 text-white px-4 py-2"
           href="<?= h($_SERVER['PHP_SELF'].'?'.http_build_query(array_merge($_GET,['__apply'=>1,'export'=>'pdf']))) ?>">Export PDF</a>
      </div>
    </form>
  </div>
